...
//*********************************************************  
    ...
    public class ReceiveThread implements Runnable{
        ...
        public void run() {
            /*Input Data Sample
            INPUT DATA : #bundle            D /HandRight ,ssssss     0.4715644       0.1893958       1.919592       D /HandLeft  ,ssssss     -0.2721419      0.1553801       1.781009       H /AnkleRight    ,ssssss     0.1168895       -1.085168       2.064021       D /AnkleLeft ,ssssss     -0.03268357     -1.00057        1.945235                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
            DATA FROM KINECT(C#, SharpOSC)
            objects : HandRig
            objects : HandLef
            objects : AnkleRi
            objects : AnkleLe
            The number of elements) : 4
            INPUT DATA : #bundle            D /HandRight ,ssssss     0.4703115       0.1980381       1.941879       D /HandLeft  ,ssssss     -0.2519117      0.1657319       1.794747       H /AnkleRight    ,ssssss     0.1162506       -1.090987       2.076941       D /AnkleLeft ,ssssss     -0.03322925     -0.9981002      1.947154                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
            DATA FROM KINECT(C#, SharpOSC)
            objects : HandRig
            objects : HandLef
            objects : AnkleRi
            ...
            The number of elements) : 4
            */            
            //this.decodeAndUpdate("#bundle            D /HandRight ,ssssss     0.4715644       0.1893958       1.919592       D /HandLeft  ,ssssss     -0.2721419      0.1553801       1.781009       H /AnkleRight    ,ssssss     0.1168895       -1.085168       2.064021       D /AnkleLeft ,ssssss     -0.03268357     -1.00057        1.945235                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ");
            
            //this.decodeAndUpdate("#bundle D /HandLeft ,sssss -42.8135 -45.9357 146.028"); //Data from Hifza Javed
       ...
       public void sleep(int dur){
        ...
        public void decodeAndUpdate(String input){
            ...
            //get next token & exclude header
            String source = s.next();
            String in = input.substring(source.length());
            
            //check header : OSC packets from Kinect (C#, SharpOSC)
            if(source.startsWith("#bundle")){
                //System.out.println("DATA FROM KINECT(C#, SharpOSC)");
                source = "KINECT" ;
            }else{
                
            //Original source should be here
            
            }
            
            //check source (if it is from the same source)
            boolean isSource = false;
            ...
            //get the names of the elements(objects) & init data set(list)
            s = new Scanner(in);               
            ArrayList<Object[]> data = new ArrayList<>();
            while(s.hasNext()){
                String nextS = s.next();               
                if(!nextS.trim().isEmpty()&&nextS.trim().startsWith("/")){
                    String nextObj = nextS.substring(1, 8); //Maximum length of the title(address) = 8 //nextS.indexOf(","));
                    System.out.println("objects : " + nextObj);
                    Object[] temp = {nextObj,0.0,0.0,0.0};
                    data.add(temp);
                }
            }           
            
            System.out.println("The number of objects) : " + data.size());
            
            //set the data values (obj : x, y, z)
            s = new Scanner(in);
            int oNum = 0;
            while(s.hasNext()){ 
                String nextS = s.next();
                if(nextS.trim().contains("sssss")){ //number of elements in one OSC messege including space (" ")
                    Double x = Double.valueOf(s.next().substring(0, 7));
                    Double y = Double.valueOf(s.next().substring(0, 7));
                    Double z = Double.valueOf(s.next().substring(0, 7));
            ...
            //1.New source : Init tracking data (Received data --> tracking data(Item type))
            if(!isSource){ 
                sources.add(source); 
                boolean added = false;
                if(obj[0]!=null){ 
                ...
                //Update tracking data : If it is not new
                if(!added){
                    Item[] transfer = obj;
            ...
            //2. From known source (registered in sources list)
            else{
                for(int d = 0; d < data.size(); d++){
                    for(Item i: obj){
                        if(i!=null){
                            //update same 'type (VICON extended from ITEM)', same 'elements(objects)'
